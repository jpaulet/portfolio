...
if (!function_exists("nxs_snapAjax")) { function nxs_snapAjax() { check_ajax_referer('nxsSsPageWPN'); $arg = ''; nxs_Filters::init(true);
  ...
  //## Update All
  if ($_POST['nxsact']=='setAllXS' && !empty($_POST['nid']) && !empty($_POST['uid'])) { $nid = explode('-',$_POST['nid']); $val = $networks[$nid[0]][$nid[2]][$nid[1]]; $uid = $networks[$nid[0]][$nid[2]][$_POST['uid']];      
      foreach ($networks[$nid[0]] as $ii=>$nta) if (!empty($nta[$_POST['uid']]) && $nta[$_POST['uid']]==$uid) $networks[$nid[0]][$ii][$nid[1]] = $val;            
      if (is_array($networks)) { nxs_save_ntwrksOpts($networks);  $nxs_SNAP->nxs_accts = $networks;  echo "Done"; }
  }
