...
if (!function_exists("nxs_pushPostToNT")){function nxs_pushPostToNT($ntCode, $ii, $postID, $type='a'){ $postObj = get_post($postID);
  ...
  $optNt = maybe_unserialize(get_post_meta($postID, 'snap'.$ntCode, true)); 
  $clName = 'nxs_snapClass'.strtoupper($ntCode); if (class_exists($clName)) { $cl = new $clName();
    if (empty($optNt) || !is_array($optNt)) $optNt = $networks[strtolower($ntCode)][$ii]; else $optNt = $cl->adjMetaOpt($networks[strtolower($ntCode)][$ii],$optNt[$ii]);
            ...
            if (!empty($fltrOpts)) { $fltrOpts['posts_per_page'] = '1'; 
                ...
                if (isset($rpstrOpts['rpstBtwHrsType']) && $rpstrOpts['rpstBtwHrsType']=='D'){                   
                  ...
                  if (is_array($rpstBtwDays) && count($rpstBtwDays)>0) { $currDay = (int)date_i18n('w'); if (!(in_array($currDay, $rpstBtwDays))) { // echo "D :( ";
                    nxs_LogIt('I', 'Reposter: Skipped - Excluded Day - '.$currDay, 'Reposter ID:'.$row['postid'].' (Post ID: '.$ids[0].')', '', '-=[ - ]=-', print_r($rpstrOpts, true)); continue;  
                  }}
                  //## Check Hours
                  ...
                  if ( !( ($rpstBtwHrsF<$rpstBtwHrsT && $currHour<$rpstBtwHrsT && $currHour>=$rpstBtwHrsF) || ($rpstBtwHrsF>$rpstBtwHrsT && $currHour<$rpstBtwHrsF && $currHour>=$rpstBtwHrsT) )) {  //echo "H :( ";
                    nxs_LogIt('I', 'Reposter: Skipped - Excluded Hour - '.$currHour, 'Reposter ID:'.$row['postid'].' (Post ID: '.$ids[0].')', '', '-=[ - ]=-', print_r($rpstrOpts, true)); continue;  
                  }}
                }
                ...
                if (!empty($ids[0])) { nxs_LogIt('I', 'Reposter [Time to Post 2]', 'Reposter ID:'.$row['postid'].' (Post ID: '.$ids[0].')', '', '-=[ - ]=-', print_r($rpstrOpts, true));
                    ...
                    if ($rpstrOpts['rpstStop']=='R') {  $wpdb->query( "DELETE FROM ". $wpdb->postmeta ." WHERE meta_key = 'snap_isRpstd".$pid."'" );  }
