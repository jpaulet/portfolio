...
&nbsp;&nbsp;<a style="font-size: 14px;" onclick="nxs_saveSiteSets('A', 'D');return false;" href="#">[Disable SNAP on all sites]</a><?php
  ...
  function showAccountsTab(){ global $nxs_snapAvNts, $nxsOne; $nxsOne = ''; $trrd=0; $parts = parse_url( home_url() ); $nxs_snapThisPageUrl = "{$parts['scheme']}://{$parts['host']}" . add_query_arg( NULL, NULL ); $cst=strrev('enifed'); $isMobile = nxs_isMobile();
    ...
    $nt2 = array(); foreach ($networks as $ntnt=>$ntas) foreach ($ntas as $ntii=>$nta) if (isset($ntii)&&$ntii!=='') $nt2[$ntnt][$ntii] = $nta; if ($nt2!=$networks) { $networks = $nt2; $this->saveNetworksOptions($nt2); }
       ...
       <?php /* ################## WHAT URL to USE */ ###################### ?>
          ...
          foreach ($nxs_snapAvNts as $avNt) { $clName = 'nxs_snapClass'.$avNt['code'];
             ...
             $cbo = 0; $cboInx = 0; $jXj = 0; foreach ($ntClInst->nt as $indx=>$pbo){ 
               ...
               $pbo['hideMe'] = $options['hideUnchecked'] && (empty($pbo['do']) || $fltPostTypeExcl ); if (!$pbo['hideMe']) $jXj++;  
             } if ($cbo==0) continue; if (!$this->sMode['t']) $cbo = 1;
             if ($cbo==1) { ?>
               <?php  $pbo = $ntClInst->nt[$cboInx]; $pbo['jj']=$cboInx; $pbo['cbo']=$cbo; $pbo['hideMe'] = $options['hideUnchecked'] && (empty($pbo['do']) || $fltPostTypeExcl ); 
             ...
             <div class="nxs_box" <?php if ($jXj==0) echo ' style="display:none;" '; ?> onmouseover="jQuery('.selAll<?php echo $avNt['code']; ?>').show();" onmouseout="jQuery('.selAll<?php echo $avNt['code']; ?>').hide();">
               ...
               <div class="nxs_box_inside"><?php $jj = 0;  if(!$ntClInst->checkIfFunc()) echo $ntClInst->noFuncMsg; //### List of accountns
                     ...
                     $pbo['hideMe'] = $options['hideUnchecked'] && (empty($pbo['do']) || $fltPostTypeExcl ); if (!$pbo['hideMe']) $jj++;
