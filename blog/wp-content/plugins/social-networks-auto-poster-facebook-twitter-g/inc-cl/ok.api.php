...
if (!class_exists("nxs_class_SNAP_OK")) { class nxs_class_SNAP_OK {
    ...
    function doPostToNT($options, $message){ global $nxs_urlLen; $badOut = array('pgID'=>'', 'isPosted'=>0, 'pDate'=>date('Y-m-d H:i:s'), 'Error'=>'');
      ...
      if (!empty($options['inclTags']) && $options['inclTags']=='1') $tags = nsTrnc($message['tags'], 195, ',', ''); else $tags = ''; if (empty($options['imgSize'])) $options['imgSize'] = 'original';
      //## Make Post            
      ...
      if ($options['postType']=='I') {  $toSigStr = ''; $getArr = array('application_key'=>nxs_gak($options['appKey']), 'format'=>'json', 'gid'=>$options['gid'], 'method'=>'photosV2.getUploadUrl');
          foreach ($getArr as $k=>$v) $toSigStr .= $k.'='.$v; $secKey = md5($options['access_token'].nxs_gas($options['appSec'])); $sig = md5($toSigStr.$secKey);  $getArr['sig'] = $sig; $getArr['access_token'] = $options['access_token'];           
      ...
      $toSigStr = ''; $getArr = array('application_key'=>nxs_gak($options['appKey']), 'attachment'=>$jsonToPost, 'format'=>'json', 'gid'=>$options['gid'], 'method'=>'mediatopic.post', 'text_link_preview'=>'false', 'type'=>'GROUP_THEME');      
      foreach ($getArr as $k=>$v) $toSigStr .= $k.'='.$v; $secKey = md5($options['access_token'].nxs_gas($options['appSec'])); $sig = md5($toSigStr.$secKey);  $getArr['sig'] = $sig; $getArr['access_token'] = $options['access_token']; 
