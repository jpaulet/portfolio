...
if (!class_exists("nxs_class_SNAP_FB")) { class nxs_class_SNAP_FB {
    ...
    function doPostToNT($options, $message){ $badOut = array('Warning'=>'', 'Error'=>''); $wprg = array('sslverify'=>false, 'timeout' => 30); 
      ...
      if (!empty($options['uPass'])) {
          ...
          if (!empty($options['glpid'])) $message['glpid'] = $options['glpid'];
      
          $nt = new nxsAPI_FB(); if(!empty($options['ck'])) $nt->ck = $options['ck']; $nt->sid = array('cn'=>$options['uName'],'xs'=>$pass); $res = $nt->post($options['pgID'], $message);
