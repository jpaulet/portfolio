...
function nxs_showTimeDialog(ntii,dmode){
    jQuery('#'+ntii+'timeToRunWM').val(dmode); jQuery('#nxs_timeID').val(ntii);  var dd = jQuery('#'+ntii+'timeToRun').val(); if (!!dd) { dd = dd.split('|'); if (dmode=='A') dd = dd[dd.length-1]; else dd = dd[dmode]; if (dd=='i') {var dC=new Date(); var dd = (dC.getTime() / 1000); } nxs_fillTime(dd*1000); } jQuery.pgwModal({ target: '#showSetTimeInt', title: 'Post', maxWidth: 800, closeOnBackgroundClick : true}); 
}
